...
/* ✅ Centered Modal Styles - Matching Countdown Blue */
.modal-overlay {
position: fixed;
top: 0;
left: 0;
...
/* ✅ Improved Assets Modal - Wider and More Consistent */
.modal-card {
...
align-items: center;
padding: 16px 24px;  /* ⬅️ تباعد أكبر */
...
overflow-y: auto;
flex: 1;
padding: 16px;  /* ⬅️ تباعد أكبر */
}

/* ✅ Consistent Category Styling */
.modal-category {
display: flex;
align-items: center;
gap: 8px;
padding: 12px 16px;  /* ⬅️ تباعد أكبر */
background: rgba(77, 255, 204, 0.08);
font-weight: bold;
font-size: 11px;
color: #4dffcc;
border-radius: 8px;
margin-bottom: 8px;  /* ⬅️ مسافة أكبر */
margin-top: 12px;
...
font-size: 12px;
color: #d1d4dc;
margin-bottom: 6px;  /* ⬅️ مسافة أكبر */
...
// ✅ TradingView-like Countdown Variables
...
// ✅ Disable native wheel zoom to avoid conflict
...
// Reset interaction only when user stops
...
chartContainer.addEventListener('wheel', (e) => {
    ...
    // 🔥 Smart Zoom: Faster on strong scroll, slower on gentle scroll
    const intensity = Math.min(Math.abs(e.deltaY), 100) / 100;
    const zoomFactor = e.deltaY > 0 
        ? 1 + 0.15 * intensity  // Zoom in (faster when intensity is high)
        : 1 - 0.15 * intensity; // Zoom out (faster when intensity is high)
    ...
    // Delay reset to allow smooth interaction
...
// ✅ Search functionality for assets modal
...
eel.get_timeframes()(function(tfs) {
    // Timeframes are now handled in the modal
});
...
function updateChart(data) {
    ...
    // ✅ Update countdown with every new candle data
    if (data.last_candle_time) {
        startOrUpdateCountdown(data.last_candle_time);
    }

    // ✅ FIX: Ensure historical candles are drawn at least once
    ...
    // ✅ Allow full redraw even during interaction if needed
    if (needsFullRedraw) {
    ...
    // ✅ Update last candle even during interaction
...
chart.resize(window.innerWidth, window.innerHeight - 30);
});
